...
static unsigned int kmp_find(struct ts_context *tsc, struct ts_state *tss)
{
    ...
    bool icase = !!(tsc->flags & TS_IGNORECASE);
    struct kmp_context *kctx = ts_to_kmp(tsc);
...
static struct ts_context *kmp_prepare(const void *pattern, size_t len, unsigned long flags)
{
    unsigned int prefix_size = sizeof(unsigned int) * len;
    struct kmp_context *kctx;
    unsigned int index;

    kctx = malloc(sizeof(*kctx) + prefix_size + len);
    if (unlikely(!kctx))
        return NULL;
